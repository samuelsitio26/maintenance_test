<script>
    ...
    const statusPengembalianOptions = [{
            value: '',
            label: 'Semua Status Pengembalian'
        },
        {
            value: 'tepat_waktu',
            label: 'Tepat Waktu'
    ...
    function filterRentals() {
        filteredRentals = rentals.filter((rental) => {
            ...
            const matchesStatus = !statusFilter || getStatusLabel(rental).toLowerCase() === statusFilter
                .toLowerCase();
            const matchesStatusPengembalian = !statusPengembalianFilter ||
                rental.status_pengembalian?.toLowerCase() === statusPengembalianFilter.toLowerCase();
            const matchesKondisi = !kondisiFilter || rental.kondisi?.toLowerCase() === kondisiFilter
                .toLowerCase();
...
<div class="mx-auto px-4 py-8" style="max-width:1600px; font-size:1.1rem; margin-left:-50px; margin-right:-50px;">
    <!-- Header -->
    ...
    <div class="bg-white rounded-2xl shadow border border-gray-200 px-4 py-8 min-h-[520px] max-h-[800px]"
        style="margin-left:-20px; margin-right:-20px;">
        <div class="flex flex-row gap-8 xl:gap-12">
            <!-- Panel Kiri: Daftar Peminjaman -->
            <div class="flex flex-col h-full min-h-0" style="width:38%; min-width:420px; max-width:520px;">
                <!-- Filter & Search -->
                <div class="flex flex-col gap-3 mb-4 p-4 bg-white rounded-xl border border-gray-200 shadow-sm">
                    <div class="flex flex-wrap gap-2 items-center">
                        <input type="date"
                            ...
                            placeholder="Dari" style="width: 150px;" />
                        <span class="text-gray-400 text-sm">sampai</span>
                        <input type="date"
                            class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                            placeholder="Sampai" style="width: 150px;" />
                    </div>
                    <div class="flex flex-wrap gap-2 items-center">
                        ...
                        <input type="text" bind:value={searchTerm} placeholder="Cari barang/peminjam..."
   ...
   <div class="p-6">
    ...
    <div class="space-y-4">
     ...
     <div>
      ...
      >
      <textarea id="return-comment" bind:value={returnComment} placeholder="Tambahkan catatan pengembalian..."
          class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          rows="3"></textarea>
     </div>
    </div>
